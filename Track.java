...
class Track {
	...
	public Track(String url, Map<String, String> metadata) {
		...
		if (metadata.get("img") == null) {
			if (metadata.get("mbid_artist") != null) metadata.put("img", "http://static.bbc.co.uk/music/images/artists/542x305/"+metadata.get("mbid_artist")+".jpg");
			else metadata.put("img", Manager.getSetting("default_img"));
		}
		if (metadata.get("thumb") == null) {
			if (metadata.get("mbid_artist") != null) metadata.put("thumb", "http://static.bbc.co.uk/music/images/artists/234x132/"+metadata.get("mbid_artist")+".jpg");
		...
		if (metadata.get("exturl") == null) {
			if (metadata.get("pid") != null) metadata.put("exturl", "http://www.bbc.co.uk/programmes/"+metadata.get("pid"));
			else if(metadata.get("mbid_artist") != null) metadata.put("exturl", "http://www.bbc.co.uk/music/artists/"+metadata.get("mbid_artist"));
		}
