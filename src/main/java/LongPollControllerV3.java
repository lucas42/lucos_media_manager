...
class LongPollControllerV3 extends Controller {
	int POLL_TIMEOUT; // Number of seconds before the Long Poll should return, even if no changes have
						// happened
	int DEVICE_CONNECTION_TIDYUP; // Number of seconds after the request ends before tidying up the request in the
									// device list
	...
	// Overwrites the standard Controller run method to handle tidying up devices
	// afterwards
	public void run() {
		super.run();

		try {
			// Wait before marking this connection as closed, to give the client time to
			// re-establish a long poll
